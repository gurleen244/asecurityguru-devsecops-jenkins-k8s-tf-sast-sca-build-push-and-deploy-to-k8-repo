pipeline {
    ...
    stages {
        ...
        stage('Push to ECR') {
            steps {
                withCredentials([[
                    ...
                    script {
                        sh '''
                            aws ecr get-login-password --region $AWS_REGION | docker login --username AWS --password-stdin 982081064548.dkr.ecr.$AWS_REGION.amazonaws.com
        ...
        stage('Kubernetes Deployment of ASG Bugg Web Application') {
            steps {
                withKubeConfig([credentialsId: 'kubectl_cred']) {
